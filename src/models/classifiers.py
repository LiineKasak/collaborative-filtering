...
class svdInputClassifier(AlgoBase):
    def __init__(self, clf=None, track_to_comet=False, method_name=None, api_key="rISpuwcLQoWU6qan4jRCAPy5s",
        ...
        self.sgd = ADAPTED_SVD_SGD(verbal=True, epochs=25, k_singular_values=12)
    ...
    def fit(self, datawrapper, val_users=None, val_movies=None):
        ...
        self.user_embeddings, self.movie_embeddings = self.sgd.pu, self.sgd.qi
    ...
    def get_features(self, users, movies):
        user_features = self.user_embeddings[users]
        movie_features = self.movie_embeddings[movies]
