...
def fit_and_score(train_index, test_index):
    ...
    (ur_val, _) = create_adjacency_lists(val_users, val_movies, val_predictions)
    ...
    (bu_1, bi_1, pu, qi, global_mean) = fit_model_svd(bu, bi, ur, ir, train_users, train_movies, train_predictions, 1)
...
test_data_pd = pd.read_csv('sampleSubmission.csv')
test_users, test_movies, test_predictions = extract_users_items_predictions(test_data_pd)
...
print('#############################   starting fit... #############################')
(bu, bi, global_mean) = fit_model_baseline_als(ur, ir, train_users, train_movies, train_predictions, 1)
(bu1, bi2, pu, qi, global_mean) = fit_model_svd(bu, bi, ur, ir, train_users, train_movies, train_predictions, 1)
print('#############################   start predictions... #############################')
test_predictions = predict_svd(ur, ir, train_users, train_movies, test_users, test_movies, bu, bi, pu, qi, global_mean)

test_predictions = mean_u_denormalize(u_means, u_vars, test_users, test_predictions)


print('#############################   create sub file... #############################')
import os
...
def create_submission_file(sub_users, sub_movies, predictions, name='submission'):
    ...
    print(directory_path + name + '.csv.zip')
    # export to file:
    # archive_name is required to create working zip on my computer
    pred_pd.to_csv(directory_path + name + '.csv.zip',
                   index=False,
                   compression={'method': 'zip', 'archive_name': name + '.csv'}
                   )


create_submission_file(test_users, test_movies, test_predictions, name='submission3')
